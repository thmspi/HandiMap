...
router.get('/avis/etablissement/:id', (req, res) => {
  try {
    ...
    console.log(`Consultation des avis pour l'établissement ${etablissementId}`);
    
    // Lire tous les avis
    const allAvis = readAvis();
    
    // Filtrer pour cet établissement
    const avisEtablissement = allAvis.filter(avis => 
      avis.etablissement_id === etablissementId
    );
